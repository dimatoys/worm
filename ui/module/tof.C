...
int rc;
	...
	//printf("readReg16_w: rc=%d (1) %X\n", rc, (uint32_t)ucAddr);
	if (rc == 1)
	{
		rc = read(file_i2c, ucTemp, 2);
		//printf("readReg16_r: rc=%d (2) %X\n", rc, (uint32_t)ucAddr);
	}
...
int rc;
        ...
        //printf("readReg_w: rc=%d (1) %X\n", rc, (uint32_t)ucAddr);
        
	if (rc == 1)
	{
        	rc = read(file_i2c, &ucTemp, 1);
        	//printf("readReg_r: rc=%d (1) %X\n", rc, (uint32_t)ucAddr);
...
int rc;
	...
	rc = write(file_i2c, &ucAddr, 1);
	//printf("readMulti_w: rc=%d (1) %X\n", rc, (uint32_t)ucAddr);
	if (rc == 1)
	{
		rc = read(file_i2c, pBuf, iCount);
		//printf("readMulti_r: rc=%d (%d) %X\n", rc, iCount, (uint32_t)ucAddr);
		if (rc != iCount) {};
	}
} /* readMulti() */
...
int rc;
	...
	if (rc != iCount+1) {};
	//printf("writeMulti: rc=%d (%d) %X cnt=%d\n", rc, iCount+1, (uint32_t)ucAddr, iCount);
} /* writeMulti() */
//
...
int rc;
	...
	//printf("writeReg16: rc=%d (3) %X val=%u\n", rc, (uint32_t)ucAddr, (uint32_t)usValue);
} /* writeReg16() */
//
...
int rc;
	...
	//printf("writeReg: rc=%d (2) %X val=%u\n", rc, (uint32_t)ucAddr, (uint32_t)ucValue);
} /* writeReg() */
...
int rc;
	...
	while (ucCount)
	{
		...
		if (rc != 2) {};
		//printf("writeRegList: rc=%d (2) val=%X\n", rc, (uint32_t)*ucList);
		ucList += 2;
		ucCount--;
	}
} /* writeRegList() */
...
static void writeReg32(unsigned char ucAddr, uint32_t usValue) {

unsigned char ucTemp[5];
int rc;

	ucTemp[0] = ucAddr;
	ucTemp[1] = (unsigned char)(usValue >> 24) & 0xFF; // MSB first
	ucTemp[2] = (unsigned char)(usValue >> 16) & 0xFF;
	ucTemp[3] = (unsigned char)(usValue >> 8)  & 0xFF; // MSB first
	ucTemp[4] = (unsigned char)usValue & 0xFF;
	rc = write(file_i2c, ucTemp, 5);
	if (rc != 5) {}; // suppress warning
	//printf("writeReg16: rc=%d (3) %X val=%u\n", rc, (uint32_t)ucAddr, (uint32_t)usValue);
}
...
void startContinuous(uint32_t periodMilliseconds) {
	writeReg(0x80, 0x01);
	writeReg(0xFF, 0x01);
	writeReg(0x00, 0x00);
	...
	if (periodMilliseconds != 0) {
		...
		uint16_t oscCalibrateValue = readReg16(OSC_CALIBRATE_VAL);

		if (oscCalibrateValue != 0) {
			periodMilliseconds *= oscCalibrateValue;
		}

		writeReg32(SYSTEM_INTERMEASUREMENT_PERIOD, periodMilliseconds);
